...
class TadoDriver extends OAuth2Driver {
  ...
  static getZoneIcon({ home, zone }) {
    ...
    switch (zone.type) {
      ...
      case 'HEATING':
        const { deviceTypes } = zone;
        if (deviceTypes.length) {
          let deviceString = '.';
          deviceTypes.forEach((item, index) => {
            deviceString += item;
          });
          const xRU = deviceString.includes('RU');	// Thermostat available?
          const xVA = deviceString.includes('VA');	// Radiator thermostat available?
          const xVA2 = deviceString.includes('VA', xVA + 1);	// more Radiator thermostats available?
  ...
  static getZoneCapabilitiesOptions({ home, zone }) {
    ...
    let tMin = 16; let tMax = 30; // AC defaults
    switch (mapType) {
      case 'HEATING':
      ...
      case 'AIR_CONDITIONING':
        // var tMin = 16; var tMax = 30; //AC defaults
